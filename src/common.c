...
int get_addr(char *host, int port, int addr_family, int flags, struct sockaddr_storage *out) {
...
int open_dns(struct sockaddr_storage *sockaddr, size_t sockaddr_len) {
...
void read_password(char *buf, size_t len) {
    ...
    for (i = 0; i < sizeof(pwd); i++) {
        ...
        if (pwd[i] == '\r' || pwd[i] == '\n') {
            ...
            i--; /* Remove the \b char */
            if (i >= 0)
                i--; /* If not first char, remove one more */
        }
    }
#endif
    fprintf(stderr, "\n");
...
int check_topdomain(char *str, char **errormsg) {
    int i;
    int dots = 0;
    int chunklen = 0;
    ...
    if (strlen(str) < 3) {
        if (errormsg)
            *errormsg = "Too short (< 3)";
        return 1;
    }
    if (strlen(str) > 128) {
        if (errormsg)
            *errormsg = "Too long (> 128)";
        return 1;
    }

    if (str[0] == '.') {
        if (errormsg)
            *errormsg = "Starts with a dot";
        return 1;
    }
    ...
    for (i = 0; i < strlen(str); i++) {
        if (str[i] == '.') {
            ...
            if (chunklen > 63) {
                if (errormsg)
                    *errormsg = "Too long domain part (> 63)";
                return 1;
            }
            chunklen = 0;
        } else {
        ...
        if ((str[i] >= 'a' && str[i] <= 'z') || (str[i] >= 'A' && str[i] <= 'Z') || isdigit(str[i]) || str[i] == '-' ||
            str[i] == '.') {
            continue;
        } else {
            if (errormsg)
                *errormsg = "Contains illegal character (allowed: [a-zA-Z0-9-.])";
            return 1;
        }
    }
