...
DWORD WINAPI tun_reader(LPVOID arg) {
    ...
    while (TRUE) {
        ...
        if (!res) {
            ...
            res = sendto(sock, buf, len, 0, (struct sockaddr *) &(tun->addr), tun->addrlen);
        }
    }
...
int open_tun(const char *tun_device) {
    ...
    dev_handle = CreateFile(tapfile, GENERIC_WRITE | GENERIC_READ, 0, 0, OPEN_EXISTING,
 ...
 * (n + 1) for manual.
 */
static int utun_unit(const char *dev) {
...
static int open_utun(const char *dev) {
...
int tun_setip(const char *ip, const char *other_ip, int netbits) {
    ...
    display_ip = ip;
#endif
    snprintf(cmdline, sizeof(cmdline), IFCONFIGPATH "ifconfig %s %s %s netmask %s", if_name, ip, display_ip,
             inet_ntoa(net));
    ...
    if (r != 0) {
        ...
        snprintf(cmdline, sizeof(cmdline), "/sbin/route add %s/%d %s", inet_ntoa(netip), netbits, ip);
    }
    ...
    status = 1;
    r = DeviceIoControl(dev_handle, TAP_IOCTL_SET_MEDIA_STATUS, &status, sizeof(status), &status, sizeof(status), &len,
                        NULL);
    if (!r) {
    ...
    r = DeviceIoControl(dev_handle, TAP_IOCTL_CONFIG_TUN, &ipdata, sizeof(ipdata), &ipdata, sizeof(ipdata), &len, NULL);
    if (!r) {
    ...
    snprintf(cmdline, sizeof(cmdline), "netsh interface ip set address \"%s\" static %s %s", if_name, ip,
             inet_ntoa(net));
    return system(cmdline);
#endif
}
...
int tun_setmtu(const unsigned mtu) {
    ...
    if (mtu > 200 && mtu <= 1500) {
        snprintf(cmdline, sizeof(cmdline), IFCONFIGPATH "ifconfig %s mtu %u", if_name, mtu);
