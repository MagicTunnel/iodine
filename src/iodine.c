...
static void usage() {
    ...
    fprintf(stderr, "Usage: %s [-v] [-h] [-f] [-r] [-u user] [-t chrootdir] [-d device] "
                    "[-P password] [-m maxfragsize] [-M maxlen] [-T type] [-O enc] [-L "
                    "0|1] [-I sec] "
...
static void help() {
    ...
    fprintf(stderr, "  -T force dns type: NULL, PRIVATE, TXT, SRV, MX, CNAME, A "
                    "(default: autodetect)\n");
    fprintf(stderr, "  -O force downstream encoding for -T other than NULL: "
                    "Base32, Base64, Base64u,\n");
    fprintf(stderr, "     Base128, or (only for TXT:) Raw  (default: autodetect)\n");
    fprintf(stderr, "  -I max interval between requests (default 4 sec) to "
                    "prevent DNS timeouts\n");
    fprintf(stderr, "  -L 1: use lazy mode for low-latency (default). 0: don't "
                    "(implies -I1)\n");
    ...
    fprintf(stderr, "  -z context, to apply specified SELinux context after "
                    "initialization\n");
    fprintf(stderr, "  -F pidfile to write pid to a file\n");
    fprintf(stderr, "nameserver is the IP number/hostname of the relaying "
                    "nameserver. if absent, /etc/resolv.conf is used\n");
...
int main(int argc, char **argv) {
    ...
    if (nameserv_host) {
        ...
        if (nameservaddr_len < 0) {
            errx(1, "Cannot lookup nameserver '%s': %s ", nameserv_host, gai_strerror(nameservaddr_len));
        }
