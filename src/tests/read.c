...
START_TEST(test_read_putshort) {
    ...
    for (i = 0; i < 65536; i++) {
        p = (char *) &k;
        putshort(&p, i);
        fail_unless(ntohs(k) == i, "Bad value on putshort for %d: %d != %d", i, ntohs(k), i);

        p = (char *) &k;
        readshort(NULL, &p, &l);
        fail_unless(l == i, "Bad value on readshort for %d: %d != %d", i, l, i);
    }
}
END_TEST
...
START_TEST(test_read_putlong) {
    ...
    for (i = 0; i < 32; i++) {
        ...
        fail_unless(ntohl(k) == j, "Bad value on putlong for %d: %d != %d", i, ntohl(j), j);

        p = (char *) &k;
        readlong(NULL, &p, &l);

        fail_unless(l == j, "Bad value on readlong for %d: %d != %d", i, l, j);
    }
}
END_TEST

START_TEST(test_read_name_empty_loop) {
    unsigned char emptyloop[] = {'A',  'A',  0x81, 0x80, 0x00, 0x01, 0x00, 0x00, 0x00,
...
START_TEST(test_read_name_inf_loop) {
    unsigned char infloop[] = {'A',  'A',  0x81, 0x80, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
                               0x00, 0x00, 0x01, 'A',  0xc0, 0x0c, 0x00, 0x01, 0x00, 0x01};
    char buf[1024];
    char *data;
    int rv;
...
START_TEST(test_read_name_longname) {
    unsigned char longname[] = "AA\x81\x80\x00\x01\x00\x00\x00\x00\x00\x00"
...
START_TEST(test_read_name_onejump) {
    unsigned char onejump[] = "AA\x81\x80\x00\x01\x00\x00\x00\x00\x00\x00"
...
START_TEST(test_read_name_badjump_start) {
    unsigned char badjump[] = {'A',  'A',  0x81, 0x80, 0x00, 0x01, 0x00, 0x00, 0x00,
                               0x00, 0x00, 0x00, 0xfe, 0xcc, 0x00, 0x01, 0x00, 0x01};
...
START_TEST(test_read_name_badjump_second) {
    unsigned char badjump2[] = {'A',  'A',  0x81, 0x80, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
                                0x00, 0x02, 'B',  'A',  0xfe, 0xcc, 0x00, 0x01, 0x00, 0x01};
    unsigned char *jumper;
    char buf[1024];
    char *data;
    int rv;
    ...
    if (jumper) {
        ...
        fail_unless(rv == 4);
        fail_unless(strcmp("BA.", buf) == 0, "buf is not BA: %s", buf);
    }
    free(jumper);
}
END_TEST
...
START_TEST(test_putname_nodot) {
    char buf[256];
    char *nodot = "ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
START_TEST(test_putname_toolong) {
    char buf[256];
    char *toolong = "ABCDEFGHIJKLMNOPQRSTUVWXYZ.ABCDEFGHIJKLMNOPQRSTUVWXYZ."
