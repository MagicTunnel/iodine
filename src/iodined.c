...
static int get_dns_fd(struct dnsfd *fds, struct sockaddr_storage *addr) {
   ...
   Returns: 1 = okay, 0 = no space. */
{
    ...
    fill = users[userid].outpacketq_nexttouse + users[userid].outpacketq_filled;
   ...
   Returns: 1 = okay, 0 = no packets were waiting. */
{
    ...
    start_new_outpacket(userid, users[userid].outpacketq[use].data, users[userid].outpacketq[use].len);
   ...
   a new query. */
{
    ...
    for (i = 0; i < DNSCACHE_LEN; i++) {
        ...
        if (users[userid].dnscache_q[use].type != q->type || strcmp(users[userid].dnscache_q[use].name, q->name))
            continue;
        ...
        write_dns(dns_fd, q, users[userid].dnscache_answer[use], users[userid].dnscache_answerlen[use],
...
static inline void save_to_qmem(unsigned char *qmem_cmc, unsigned short *qmem_type, int qmem_len, int *qmem_lastfilled,
                                unsigned char *cmc_to_add, unsigned short type_to_add)
...
static inline void save_to_qmem_pingordata(int userid, struct query *q) {
    ...
    if (q->name[0] == 'P' || q->name[0] == 'p') {
        ...
        save_to_qmem(users[userid].qmemping_cmc, users[userid].qmemping_type, QMEMPING_LEN,
                     &users[userid].qmemping_lastfilled, (void *) cmc, q->type);
    } else {
        ...
        save_to_qmem(users[userid].qmemdata_cmc, users[userid].qmemdata_type, QMEMDATA_LEN,
                     &users[userid].qmemdata_lastfilled, (void *) cmc, q->type);
    }
}

static int answer_from_qmem(int dns_fd, struct query *q, unsigned char *qmem_cmc, unsigned short *qmem_type,
...
/* Quick helper function to keep handle_null_request() clean */
{
    ...
    return answer_from_qmem(dns_fd, q, users[userid].qmemdata_cmc, users[userid].qmemdata_type, QMEMDATA_LEN,
                            (void *) cmc);
}
...
*/
{
    ...
    /* First byte is 1 bit compression flag, 3 bits upstream seqno, 4 bits
     * upstream fragment */
    pkt[0] = (1 << 7) | ((users[userid].inpacket.seqno & 7) << 4) | (users[userid].inpacket.fragment & 15);
    /* Second byte is 3 bits downstream seqno, 4 bits downstream fragment, 1 bit
     * last flag */
    pkt[1] = ((users[userid].outpacket.seqno & 7) << 5) | ((users[userid].outpacket.fragment & 15) << 1) | (last & 1);

    if (debug >= 1) {
        fprintf(stderr, "OUT  pkt seq# %d, frag %d (last=%d), offset %d, fragsize "
                        "%d, total %d, to user %d\n",
                users[userid].outpacket.seqno & 7, users[userid].outpacket.fragment & 15, last,
                users[userid].outpacket.offset, datalen, users[userid].outpacket.len, userid);
    }
...
typedef enum { VERSION_ACK, VERSION_NACK, VERSION_FULL } version_ack_t;
...
*/
{
    ...
    if (users[userid].outpacket.seqno != down_seq || users[userid].outpacket.fragment != down_frag)
...
static void handle_null_request(int tun_fd, int dns_fd, struct dnsfd *dns_fds, struct query *q, int domain_len) {
    ...
    if (in[0] == 'V' || in[0] == 'v') {
        ...
        if (read > 4) {
            ...
            version = (((unpacked[0] & 0xff) << 24) | ((unpacked[1] & 0xff) << 16) | ((unpacked[2] & 0xff) << 8) |
                       ((unpacked[3] & 0xff)));
        }
        ...
        if (check_user_and_ip(userid, q) != 0) {
            ...
            syslog(LOG_WARNING, "dropped login request from user #%d from unexpected source %s", userid,
            ...
            if (read >= 18 && (memcmp(logindata, unpacked + 1, 16) == 0)) {
                ...
                syslog(LOG_WARNING, "rejected login request from user #%d from %s, bad password", userid,
                       format_addr(&q->from, q->fromlen));
            }
        }
        return;
        ...
        switch (in[1]) {
            case 'T':
            case 't':
                if (q->type == T_TXT || q->type == T_SRV || q->type == T_MX || q->type == T_CNAME || q->type == T_A) {
        ...
        if (answer_from_qmem(dns_fd, q, users[userid].qmemping_cmc, users[userid].qmemping_type, QMEMPING_LEN,
                             (void *) unpacked))
            return;

        /* Check if duplicate of waiting queries; impatient DNS relays
           like to re-try early and often (with _different_ .id!)  */
        if (users[userid].q.id != 0 && q->type == users[userid].q.type && !strcmp(q->name, users[userid].q.name) &&
            users[userid].lazy) {
            /* We have this ping already, and it's waiting to be
               ...
               have triggered.) */
            if (debug >= 2) {
                fprintf(stderr, "PING pkt from user %d = dupe from impatient DNS "
                                "server, remembering\n",
                        userid);
            }
        ...
        if (users[userid].q_sendrealsoon.id != 0 && q->type == users[userid].q_sendrealsoon.type &&
        ...
        if (debug >= 3) {
            fprintf(stderr, "PINGret (if any) will ack upstream %d/%d\n", users[userid].inpacket.seqno,
                    users[userid].inpacket.fragment);
        }
        ...
        if ((!didsend && users[userid].outpacket.len > 0) || !users[userid].lazy)
            send_chunk_or_dataless(dns_fd, userid, &users[userid].q);
        ...
        if (users[userid].q.id != 0 && q->type == users[userid].q.type && !strcmp(q->name, users[userid].q.name) &&
            ...
            /* We have this packet already, and it's waiting to be
               ...
               have triggered.) */
            if (debug >= 2) {
                fprintf(stderr, "IN   pkt from user %d = dupe from impatient DNS "
                                "server, remembering\n",
                        userid);
            }
            users[userid].q.id2 = q->id;
        ...
        if (upstream_ok) {
            ...
            if (debug >= 1) {
                fprintf(stderr, "IN   pkt seq# %d, frag %d (last=%d), fragsize %d, "
                                "total %d, from user %d\n",
         ...
         */
        if (users[userid].q.id != 0) {
            if ((users[userid].outpacket.len > 0 && !didsend) || (upstream_ok && !lastfrag && !didsend) ||
                (!upstream_ok && !didsend) || !users[userid].lazy) {
                didsend = 1;
                if (send_chunk_or_dataless(dns_fd, userid, &users[userid].q) == 1)
                    /* new packet from queue, send immediately */
                    didsend = 0;
...
static int tunnel_bind(int bind_fd, struct dnsfd *dns_fds) {
    char packet[64 * 1024];
...
static int tunnel_dns(int tun_fd, int dns_fd, struct dnsfd *dns_fds, int bind_fd) {
    struct query q;
    int read;
    int domain_len;
    ...
    if (inside_topdomain) {
        ...
        if (domain_len == 3 && q.type == T_A && (q.name[0] == 'n' || q.name[0] == 'N') &&
            (q.name[1] == 's' || q.name[1] == 'S') && q.name[2] == '.') {
        ...
        /* Handle A-type query for www.topdomain, for anyone that's
           poking around */
        if (domain_len == 4 && q.type == T_A && (q.name[0] == 'w' || q.name[0] == 'W') &&
            (q.name[1] == 'w' || q.name[1] == 'W') && (q.name[2] == 'w' || q.name[2] == 'W') && q.name[3] == '.') {
...
static int tunnel(int tun_fd, struct dnsfd *dns_fds, int bind_fd, int max_idle_time) {
    struct timeval tv;
    fd_set fds;
    int i;
    int userid;
    ...
    while (running) {
        ...
        if (i == 0) {
            if (max_idle_time) {
                /* only trigger the check if that's worth ( ie, no need to loop over if
                there
        ...
        for (userid = 0; userid < created_users; userid++)
            if (users[userid].active && !users[userid].disabled && users[userid].last_pkt + 60 > time(NULL) &&
                users[userid].q_sendrealsoon.id != 0 && users[userid].conn == CONN_DNS_NULL &&
    ...
    if (ret == Z_OK) {
        ...
        if (touser == -1) {
            ...
            if (users[touser].conn == CONN_DNS_NULL) {
                ...
                int dns_fd = get_dns_fd(dns_fds, &users[touser].q.from);
                send_raw(dns_fd, users[userid].inpacket.data, users[userid].inpacket.len, touser, RAW_HDR_CMD_DATA,
                         &users[touser].q);
            }
        }
    } else {
...
static void handle_raw_login(char *packet, int len, struct query *q, int fd, int userid) {
    ...
    /* can't use check_authenticated_user_and_ip() since IP address will be
       different,
...
static void handle_raw_data(char *packet, int len, struct query *q, struct dnsfd *dns_fds, int tun_fd, int userid) {
    if (check_authenticated_user_and_ip(userid, q) != 0) {
        return;
    }
...
static int raw_decode(char *packet, int len, struct query *q, int dns_fd, struct dnsfd *dns_fds, int tun_fd) {
    int raw_user;
...
static void help() {
    ...
    fprintf(stderr, "     (using -DD in UTF-8 terminal: \"LC_ALL=C luit iodined "
                    "-DD ...\")\n");
