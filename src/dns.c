...
#define CHECKLEN(x)                          \
    if (buflen < (x) + (unsigned) (p - buf)) \
    return 0

int dns_encode(char *buf, size_t buflen, struct query *q, qr_t qr, char *data, size_t datalen) {
    HEADER *header;
    short name;
    char *p;
    int len;
...
#define CHECKLEN(x)                                   \
    if (packetlen < (x) + (unsigned) (data - packet)) \
    return 0
        ...
        case QR_ANSWER:
            ...
            if (type == T_NULL || type == T_PRIVATE) {
                ...
                for (i = 0; i < ancount; i++) {
                    ...
                    if (pref % 10 == 0 && pref >= 10 && pref < 2500) {
                        readname(packet, packetlen, &data, names[pref / 10 - 1], QUERY_NAME_SIZE - 1);
