...
void client_set_nameserver(struct sockaddr_storage *addr, int addrlen) {
...
char *client_get_qtype() {
    ...
    if (do_qtype == T_NULL)
        c = "NULL";
    else if (do_qtype == T_PRIVATE)
        c = "PRIVATE";
    else if (do_qtype == T_CNAME)
        c = "CNAME";
    else if (do_qtype == T_A)
        c = "A";
    else if (do_qtype == T_MX)
        c = "MX";
    else if (do_qtype == T_SRV)
        c = "SRV";
    else if (do_qtype == T_TXT)
        c = "TXT";
    ...
    if (send_query_sendcnt >= 0 && send_query_sendcnt < 100 && lazymode) {
        ...
        if ((send_query_sendcnt > 6 && send_query_recvcnt <= 0) ||
            ...
            if (selecttimeout > 1) {
                ...
                warnx("Receiving too few answers. Will try to switch lazy mode off, "
                      "but that may not always work any more. Start with -L0 next time "
                      "on this network.");
                lazymode = 0;
                selecttimeout = 1;
...
static void send_packet(int fd, char cmd, const char *data, const size_t datalen) {
    char buf[4096];

    buf[0] = cmd;

    build_hostname(buf + 1, sizeof(buf) - 1, data, datalen, topdomain, b32, hostname_maxlen);
    send_query(fd, buf);
}
...
static void send_chunk(int fd) {
    ...
    outpkt.sentlen = build_hostname(buf + 5, sizeof(buf) - 5, p, avail, topdomain, dataenc, hostname_maxlen);
    ...
    buf[2] = b32_5to8(code); /* Third byte is 2 bits lower fragment count, 3 bits
                                downstream packet seqno */

    code = ((inpkt.fragment & 15) << 1) | (outpkt.sentlen == avail);
    buf[3] = b32_5to8(code); /* Fourth byte is 4 bits downstream fragment count, 1 bit
                                last frag flag */
...
*/
{
    ...
    addrlen = sizeof(from);
    if ((r = recvfrom(dns_fd, data, sizeof(data), 0, (struct sockaddr *) &from, &addrlen)) < 0) {
        warn("recvfrom");
        return -1;
    }
...
static int handshake_waitdns(int dns_fd, char *buf, int buflen, char c1, char c2, int timeout)
...
*/
{
    ...
    while (1) {
        ...
        if (rv < 0 && q.rcode == NOERROR &&
            (q.name[0] == 'Y' || q.name[0] == 'y' || q.name[0] == 'V' || q.name[0] == 'v')) {
            fprintf(stderr, "Got empty reply. This nameserver may not be resolving "
                            "recursively, use another.\n");
...
static int tunnel_dns(int tun_fd, int dns_fd) {
    ...
    if (q.name[0] != 'P' && q.name[0] != 'p' && q.name[0] != userid_char && q.name[0] != userid_char2) {
        send_ping_soon = 700;
    ...
    if (read < 2) {
        ...
        if (read < 0 && q.rcode == SERVFAIL && lazymode && selecttimeout > 1) {
            if (packrecv < 500 && packrecv_servfail < 4) {
                ...
                packrecv_servfail++;
                warnx("I think %ld is too many. Setting interval to 1 to hopefully "
                      "reduce SERVFAILs. But just ignore them if data still comes "
                      "through. (Use -I1 next time on this network.)",
                      packrecv_servfail);
                selecttimeout = 1;
                send_query_sendcnt = 0;
                send_query_recvcnt = 0;
...
/* read==1 happens with "QMEM" illegal replies, caused by
   ...
   All these cases are helped by faster pinging. */
    ...
    if (read == 5 && !strncmp("BADIP", buf, 5)) {
        warnx("BADIP: Server rejected sender IP address (maybe iodined -c will "
              "help), or server kicked us due to timeout. Will exit if no "
              "downstream data is received in 60 seconds.");
        return -1; /* nothing done */
    }
    ...
    if (read > 2 && new_down_seqno != inpkt.seqno && recent_seqno(inpkt.seqno, new_down_seqno)) {
    ...
    if (q.id != chunkid && q.id != chunkid_prev && q.id != chunkid_prev2) {
        ...
        if (lazymode && packrecv < 1000 && packrecv_oos == 5) {
            if (selecttimeout > 1)
                warnx("Hmm, getting some out-of-sequence DNS replies. Setting interval "
                      "to 1 (use -I1 next time on this network). If data traffic still "
                      "has large hiccups, try if -L0 works better.");
            else
                warnx("Hmm, getting some out-of-sequence DNS replies. If data traffic "
                      "often has large hiccups, try running with -L0 .");
            selecttimeout = 1;
            send_query_sendcnt = 0;
            send_query_recvcnt = 0;
        }
    ...
    while (read > 2) {
        ...
        if (new_down_seqno != inpkt.seqno) {
            ...
            inpkt.len = 0;
        } else if (inpkt.fragment == 0 && new_down_fragment == 0 && inpkt.len == 0) {
...
/* already checked read>=2 */
        ...
        if (up_ack_seqno == outpkt.seqno && up_ack_fragment == outpkt.fragment) {
...
static void send_fragsize_probe(int fd, int fragsize) {
    ...
    build_hostname(buf + 5, sizeof(buf) - 5, probedata, sizeof(probedata), topdomain, dataenc, hostname_maxlen);
...
static void send_version(int fd, uint32_t version) {
    char data[6];
...
static void send_ip_request(int fd, int userid) {
    char buf[512] = "i____.";
...
static void send_raw_udp_login(int dns_fd, int userid, int seed) {
    char buf[16];
...
static void send_downenctest(int fd, char downenc, int variant, char *s, int slen)
...
static void send_codec_switch(int fd, int userid, int bits) {
    char buf[512] = "s_____.";
    buf[1] = b32_5to8(userid);
    buf[2] = b32_5to8(bits);
...
static void send_downenc_switch(int fd, int userid) {
    char buf[512] = "o_____.";
    buf[1] = b32_5to8(userid);
...
static void send_lazy_switch(int fd, int userid) {
    char buf[512] = "o_____.";
    buf[1] = b32_5to8(userid);
...
static int handshake_version(int dns_fd, int *seed) {
    ...
    for (i = 0; running && i < 5; i++) {
        ...
        if (read >= 9) {
            payload = (((in[4] & 0xff) << 24) | ((in[5] & 0xff) << 16) | ((in[6] & 0xff) << 8) | ((in[7] & 0xff)));
            ...
            if (strncmp("VACK", in, 4) == 0) {
                *seed = payload;
                ...
                warnx("You use protocol v 0x%08x, server uses v 0x%08x. Giving up", PROTOCOL_VERSION, payload);
                return 1;
    ...
    for (i = 0; running && i < 4; i++) {
        ...
        if (r > 0) {
            ...
            if (len >= (16 + RAW_HDR_LEN)) {
                ...
                if (memcmp(in, raw_header, RAW_HDR_IDENT_LEN) == 0 && RAW_HDR_GET_CMD(in) == RAW_HDR_CMD_LOGIN &&
...
*/
{
    ...
    for (i = 0; running && i < 3; i++) {
        ...
        if (read > 0) {
            ...
            if (in[4] == 'A') {
                fprintf(stderr, "DNS queries get changed to uppercase, keeping "
                                "upstream codec Base32\n");
                return -1;
            }
            if (in[5] == 'a') {
                fprintf(stderr, "DNS queries get changed to lowercase, keeping "
                                "upstream codec Base32\n");
                return -1;
            }
...
*/
{
    ...
    if (!running) {
        warnx("Stopped while autodetecting DNS query type (try setting manually "
              "with -T)");
        return 1; /* problem */
    }
...
static int fragsize_check(char *in, int read, int proposed_fragsize, int *max_fragsize)
/* Returns: 0: keep checking, 1: break loop (either okay or definitely wrong) */
{
    ...
    if ((in[2] & 0xff) != 107) {
        fprintf(stderr, "\n");
        warnx("corruption at byte 2, this won't work. Try -O Base32, or other -T "
              "options.");
        *max_fragsize = -1;
        return 1;
    }
...
static int handshake_autoprobe_fragsize(int dns_fd) {
    ...
    if (!running) {
        fprintf(stderr, "\n");
        warnx("stopped while autodetecting fragment size (Try setting manually "
              "with -m)");
        return 0;
    }
    if (max_fragsize <= 2) {
    ...
    if (max_fragsize < 82) {
        ...
        fprintf(stderr, "Try setting -M to 200 or lower, or try other DNS types "
                        "(-T option).\n");
    } else if (max_fragsize < 202 && (do_qtype == T_NULL || do_qtype == T_PRIVATE || do_qtype == T_TXT ||
