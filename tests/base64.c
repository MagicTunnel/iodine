...
static struct tuple {
    char *a;
    char *b;
} testpairs[TUPLES] = {{"iodinetestingtesting", "Aw8KAw4LDgvZDgLUz2rLC2rPBMC"},
                       {"abc1231", "ywjJmtiZmq"},
...
START_TEST(test_base64_encode) {
    ...
    fail_unless(strcmp(buf, testpairs[_i].b) == 0, "'%s' != '%s'", buf, testpairs[_i].b);
}
END_TEST
...
START_TEST(test_base64_decode) {
    ...
    fail_unless(strcmp(buf, testpairs[_i].a) == 0, "'%s' != '%s'", buf, testpairs[_i].a);
}
END_TEST
